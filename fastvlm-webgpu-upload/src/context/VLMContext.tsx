...
import type { InferenceResult } from "../types/metrics";
    ...
    async (onProgress?: (msg: string) => void) => {
      ...
      loadPromiseRef.current = (async () => {
        try {
          ...
          console.debug("[FastVLM] Model loaded — memory snapshot:", mem);
          setIsLoaded(true);
        } catch (e) {
  ...
  const runInference = useCallback(
    async (
      ...
      try {
        ...
        frameCaptureMs = performance.now() - captureStart;
        ...
        const streamer = new TextStreamer(processorRef.current.tokenizer!, {
          ...
          callback_function: (t: string) => {
            if (!firstTokenMarked) {
              timeToFirstTokenMs = performance.now() - generationStart;
        ...
        console.debug("[FastVLM latency]", latency);
        console.debug("[FastVLM memory]", readMemorySnapshot());

        return {
          caption: decoded[0].trim(),
          skipped: false,
          latency,
        };
      } finally {
